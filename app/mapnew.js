...
window.addEventListener("DOMContentLoaded", () => {
  ...
  var loadStates = function () {
    ...
    statesLayer.queryFeatures(query)
      .then(function (response) {
        ...
        statesLoaded = true;
        if (medianLoaded && submitBtn) submitBtn.disabled = false;
      });
  ...
  var medianEarningsUrl = "https://services.arcgis.com/P3ePLMYs2RVChkJx/ArcGIS/rest/services/ACS_Median_Earnings_by_Occupation_Boundaries/FeatureServer/0"
  const medianEarningsLayer = new FeatureLayer({
    url: medianEarningsUrl,
  ...
  var loadAdditionalData = function () {
    ...
    let additionalQuery = medianEarningsLayer.createQuery();
    additionalQuery.returnGeometry = false;
    additionalQuery.outFields = "*"
    ////execute query on your new layer
    medianEarningsLayer.queryFeatures(additionalQuery).then(function (r) {
      console.log(r)
      r.features.forEach(function (feature) {
        ...
        // the median earnings field - using B24021_001E (Median Earnings of Total Full-Time Year-Round Civilian Employed Population)
        var earnings = null;
        if ('B24021_001E' in feature.attributes) {
          earnings = feature.attributes['B24021_001E'];
        } else if ('MEDIAN_EARNINGS' in feature.attributes) { // example fallback
          earnings = feature.attributes['MEDIAN_EARNINGS'];
        } else {
          // if the expected field isn't present, log available keys to pick the correct one
          console.warn('loadAdditionalData: median earnings field not found for', abbr, 'attributes keys:', Object.keys(feature.attributes));
        }

        // set medianEarnings (may be null if no field found)
        stateObj[matchKey]['medianEarnings'] = earnings;
      });
      console.log('loadAdditionalData: median earnings loaded');
      medianLoaded = true;
      if (statesLoaded && medianLoaded) {
        if (submitBtn) submitBtn.disabled = false;
        console.log('All data loaded — submit enabled');
  ...
  formElement.addEventListener("submit", (e) => {
          ...
          if (value.notax == true) {
            $table.bootstrapTable('insertRow', {
              ...
              row: {
                ...
                medianEarnings: value.medianEarnings
              }
            })
            if (value.geometry) {
              let gfx = new Graphic({
                ...
                attributes: {
                  ...
                  "PercentOwed": 0,
                  "MedianEarnings": value.medianEarnings || 0,
                  "ObjectId": index
                }
              });
              graphics.push(gfx)
    ...
    try {
      if (map.layers && map.layers.length) {
        ...
        map.layers.forEach(function (lyr) {
          if (lyr && (lyr.title === 'US States' || lyr.title === 'State Tax Layer' || lyr.title === 'State Tax and Earnings Layer')) {
    ...
    // earnings-based choropleth renderer
    const renderer = {
      type: "class-breaks", // autocasts as new ClassBreaksRenderer()
      field: "MedianEarnings",
      legendOptions: {
        title: "Median Earnings by Occupation"
      },
      defaultSymbol: {
    ...
    const layer = new FeatureLayer({
      ...
      title: "State Tax and Earnings Layer",
      fields: [{
        name: "ObjectId",
        type: "oid"
      }, {
      ...
      popupTemplate: {
        content: "State: {State_Abbr} <br>" +
          ...
          "Income after states taxes: {Income_After_Taxes} <br>" +
          "Median Earnings by Occupation: {MedianEarnings}"
      },
      renderer: renderer

    });

    console.log('Adding State Tax and Earnings Layer to map with', graphics.length, 'graphics');
    map.add(layer);
  });
});